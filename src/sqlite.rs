...
impl SqlitePreimageStoreCursor {
    ...
    fn get_next_latest_branch(&mut self, path: Nibbles) -> ExternalStorageResult<NextBranchResult> {
        ...
        let last_current_path_key = BranchNodeKey {
            hashed_address: self.hashed_address,
            path: StoredNibbles(path),
        }
        .encode();
        ...
        let BranchNodeKey {
            hashed_address: next_trie_node_address,
            path: next_trie_node_path,
...
impl BranchNodeKey {
    ...
    fn decode(bytes: &[u8]) -> ExternalStorageResult<BranchNodeKey> {
        ...
        match prefix {
            ...
            _ => Err(ExternalStorageError::StorageError(format!(
                "invalid key prefix: {}",
                prefix
            ))),
        }
...
impl ExternalStateStore for SqlitePreimageStore {
    ...
    fn get_last_storage_leaf(&self) -> ExternalStorageResult<Option<(B256, B256)>> {
        ...
        Ok(result.map(|(hashed_address, storage_key)| {
            (
                B256::from_slice(&hashed_address),
                B256::from_slice(&storage_key),
            )
        }))
    }
    ...
    fn trie_cursor(
        ...
        Ok(SqlitePreimageStoreCursor::new(
            self.connect()?,
            hashed_address,
            max_block_number,
        ))
    ...
    fn storage_hashed_cursor(
        ...
        Ok(SqlitePreimageStoreStorageCursor::new(
            self.connect()?,
            hashed_address,
            max_block_number,
        ))
    ...
    fn account_hashed_cursor(
        ...
        Ok(SqlitePreimageStoreAccountCursor::new(
            self.connect()?,
            max_block_number,
        ))
    }
...
mod tests {
    ...
    async fn test_cursor_single_entry() {
        ...
        // Store single entry
        store
            .store_trie_branch(50, path.clone(), None, Some(branch.clone()))
            .await
            .unwrap();
    ...
    async fn test_seek_between_existing_nodes() {
        ...
        store
            .store_trie_branch(50, path1.clone(), None, Some(branch.clone()))
            .await
            .unwrap();
        store
            .store_trie_branch(50, path2.clone(), None, Some(branch.clone()))
            .await
            .unwrap();
    ...
    async fn test_same_path_different_blocks() {
        ...
        store
            .store_trie_branch(50, path.clone(), None, Some(branch1.clone()))
            .await
            .unwrap();
        store
            .store_trie_branch(100, path.clone(), None, Some(branch2.clone()))
            .await
            .unwrap();
    ...
    async fn test_deleted_branch_nodes() {
        ...
        store
            ...
            .unwrap();
        store
            .store_trie_branch(100, path.clone(), None, None)
            .await
            .unwrap();
    ...
    async fn test_account_specific_cursor() {
        ...
        store
            .store_trie_branch(50, path.clone(), Some(addr1), Some(branch.clone()))
            .await
            .unwrap();
        store
            .store_trie_branch(50, path.clone(), Some(addr2), Some(branch.clone()))
            .await
            .unwrap();
    ...
    async fn test_state_trie_cursor() {
        ...
        store
            .store_trie_branch(50, path.clone(), Some(addr), Some(branch.clone()))
            .await
            .unwrap();
        store
            .store_trie_branch(50, path.clone(), None, Some(branch.clone()))
    ...
    async fn test_mixed_account_state_data() {
        ...
        store
            .store_trie_branch(50, path1.clone(), Some(addr), Some(branch.clone()))
            .await
            .unwrap();
        store
            .store_trie_branch(50, path2.clone(), None, Some(branch.clone()))
    ...
    async fn test_store_and_retrieve_single_account() {
        ...
        // Store account
        store
            .store_hashed_accounts(vec![(account_key, Some(account.clone()))], 50)
            .await
            .unwrap();
    ...
    async fn test_storage_block_versioning() {
        ...
        store
            .store_hashed_storages(
                vec![(
                    hashed_address,
                    StorageEntry::new(storage_key, U256::from(100)),
                )],
                50,
            )
            .await
        ...
        store
            .store_hashed_storages(
                vec![(
                    hashed_address,
                    StorageEntry::new(storage_key, U256::from(200)),
                )],
                100,
            )
            .await
    ...
    async fn test_empty_cursors() {
        ...
        let mut storage_cursor = store
            .storage_hashed_cursor(B256::repeat_byte(0x01), 100)
            .unwrap();
        assert!(
            storage_cursor
                .seek(B256::repeat_byte(0x10))
                .unwrap()
                .is_none()
        );
    ...
    async fn test_cursor_boundary_conditions() {
        ...
        store
            .store_hashed_accounts(vec![(account_key, Some(account))], 50)
            .await
            .unwrap();
    ...
    async fn test_account_cursor_is_storage_empty_with_block_filtering() {
        ...
        store
            .store_hashed_accounts(vec![(account_key, Some(account))], 100)
            .await
            .unwrap();
    ...
    async fn test_account_cursor_is_storage_empty_after_deletion() {
        ...
        store
            ...
            .unwrap();
        store
            .store_hashed_accounts(vec![(account_key, None)], 100)
            .await
            .unwrap();
